//
//  UIFont + Extension.swift
//  Spinx allp
//
//  Created by SV on 15.04.2022.
//

import UIKit

extension UIFont {
    
    /// Medium roboto 24
    static func robotoMedium24() -> UIFont? {
        UIFont.init(name: "Roboto-Medium", size: 24)
    }
    /// Medium roboto 12
    static func robobtMedium12() -> UIFont? {
        UIFont.init(name: "Roboto-Medium", size: 12)
    }
    /// Medium roboto 14
    static func robotoMedium14() -> UIFont? {
        UIFont.init(name: "Roboto-Medium", size: 14)
    }
    /// Medium roboto 18
    static func robotoMedium18() -> UIFont? {
        UIFont.init(name: "Roboto-Medium", size: 18)
    }
    /// Medium roboto 35
    static func robotoMedium35() -> UIFont? {
        UIFont.init(name: "Roboto-Medium", size: 35)
    }
    /// Bold roboto 16
    static func robotoBold16() -> UIFont? {
        UIFont.init(name: "Roboto-Bold", size: 16)
    }
    /// Bold roboto 20
    static func robotoBold20() -> UIFont? {
        UIFont.init(name: "Roboto-Bold", size: 20)
    }
    /// Bold roboto 30
    static func robotoBold30() -> UIFont? {
        UIFont.init(name: "Roboto-Bold", size: 30)
    }
    /// Bold roboto 45
    static func robotoBold45() -> UIFont? {
        UIFont.init(name: "Roboto-Bold", size: 45)
    }
    /// Bold roboto 45
    static func robotoBlack45() -> UIFont? {
        UIFont.init(name: "Roboto-Black", size: 45)
    }
    /// Bold roboto 50
    static func robotoBlack50() -> UIFont? {
        UIFont.init(name: "Roboto-Black", size: 50)
    }
}
